...
parser.add_option("-s", "--serverUrl",
                  ...
                  help="connect to specified commentchaind server URL")
...
### connect to commentchaind

commentchain = AuthServiceProxy(options.serverUrl)
lastK = -1
lastUserPost = commentchain.getposts(1, [{"username":username}])
...
for i in xrange(len(items)-1,0,-1):
    ...
    if datatime > db.lastDatatime :
        ...
        if not options.dryRun:
            try:
                commentchain.newpostmsg(username, lastK+1, ptext)
            except:
                pass
        lastK = lastK+1
