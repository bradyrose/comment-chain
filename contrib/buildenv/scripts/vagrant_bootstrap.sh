...
TWISTER_CORE_PATH='/home/vagrant/commentchain-core'
TWISTER_HOME='/home/vagrant/.commentchain'
...
echo
echo 'Running bootstrap for commentchain-core'
echo "
bootstrap=$bootstrap
compile=$compile
run=$run
...
if [ ! -d "$TWISTER_HOME" ]; then
	...
	cd $TWISTER_HOME
	$AS_VAGRANT touch commentchain.conf
	echo -e "rpcuser=user\nrpcpassword=pwd\nrpcallowip=*" > commentchain.conf
	chmod 600 commentchain.conf
fi
...
if [ $run -eq 1 ]; then
	echo '.. launching commentchaind'
	cd $TWISTER_CORE_PATH
	$AS_VAGRANT -H ./commentchaind -debug -daemon
fi
...
If you want to do some development or other stuff then...
 $ vargrant ssh
 $ source commentchain-core/contrib/buildenv/scripts/activate
 
 This will give you some nice to have commands like
 * commentchain start|stop   -  to start and stop the server
